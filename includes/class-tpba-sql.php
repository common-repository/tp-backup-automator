...
class TPBA_Sql {
	...
	function get_cols( $columns, $limit = false, $offset = false, $where = false ) {
		...
		$results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM `$this->table` $wheresql $limitsql $offsetsql" ) );
	...
	// START \ CODE TP BACKUP SQL /////////////////////////////////////////////////////////////////////
	/**
	 * Get all table of Site
	 * 
	 * @global type $wpdb
	...
	public function get_all_table() {
		...
		global $wpdb;
		// List all table
		$list_table = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM information_schema.tables WHERE table_schema='%s';", $wpdb->dbname ) );
		$list = array();
		foreach ( $list_table as $table ) {
			array_push( $list, $table->TABLE_NAME );
		}
